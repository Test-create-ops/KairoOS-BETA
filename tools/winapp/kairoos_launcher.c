/*
 * KairoOS Windows launcher
 * A small Win32 GUI app that boots KairoOS inside bundled QEMU.
 * Expects, next to kairoos.exe:
 *   - qemu\qemu-system-x86_64.exe
 *   - qemu\edk2-x86_64-code.fd   (OVMF, UEFI firmware)
 *   - viteza.iso                 (KairoOS UEFI ISO)
 *
 * Cross-compile on macOS with mingw-w64:
 *   x86_64-w64-mingw32-gcc -O2 -mwindows -o kairoos.exe \
 *       kairoos_launcher.c -luser32 -lgdi32 -lcomctl32
 */
...
static char g_status[512] = "Pronto. Premi Avvia per avviare KairoOS.";
...
static void start_kairoos(void)
{
    ...
    if (g_hProc) {
        set_status("KairoOS è già in esecuzione.");
        return;
    }
    ...
    set_status("KairoOS in esecuzione... chiudi la finestra QEMU per tornare qui.");
}

static void stop_kairoos(void)
{
    if (g_hProc) {
...
static void create_controls(void)
{
    ...
    g_hBtnStart = CreateWindowExA(0, "BUTTON", "Avvia KairoOS",
...
static LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_COMMAND:
        switch (LOWORD(wp)) {
        case BTN_START: start_kairoos(); return 0;
        case BTN_STOP:  stop_kairoos();  return 0;
        case BTN_ABOUT:
            MessageBoxA(hwnd,
                "KairoOS — sistema operativo hobby x86-64.\n"
                "Gira su QEMU (TCG) con firmware UEFI/OVMF.\n\n"
                "Single-developer, fatto con passione.",
                "Informazioni su KairoOS", MB_OK | MB_ICONINFORMATION);
            return 0;
        }
        return 0;
    case WM_APP + 1:
        ...
        else    set_status("KairoOS chiuso normalmente.");
        return 0;
    case WM_CTLCOLORBTN:
    ...
    case WM_PAINT:
    {
        ...
        SetTextColor(hdc, RGB((DLG_FG>>16)&255, (DLG_FG>>8)&255, DLG_FG&255));
        TextOutA(hdc, 36, 118,
            "Avvia KairoOS in una finestra virtuale (QEMU).", 47);
        EndPaint(hwnd, &ps);
        return 0;
    }
    case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hi, HINSTANCE hp, LPSTR cmd, int show)
{
    ...
    wc.lpszClassName = "KairoOSLauncherWnd";
    RegisterClassA(&wc);

    RECT r = {0, 0, 370, 350};
    AdjustWindowRect(&r, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, FALSE);

    g_hWnd = CreateWindowExA(0, "KairoOSLauncherWnd", "KairoOS",
