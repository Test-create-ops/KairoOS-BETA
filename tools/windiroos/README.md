# WindiroOS — Windows App (.exe + Microsoft Store)

WindiroOS si distribuisce su Windows in due modi:

| File | Dove | Scopo |
|---|---|---|
| `WindiroOS-Setup.exe` | GitHub Releases | Installer (NSIS): installa WindiroOS + QEMU + ISO, scorciatoia desktop |
| `WindiroOS-x86_64.msix` | Microsoft Store | Pacchetto che Microsoft firma e ospita gratis |

WindiroOS gira dentro **QEMU** (emulatore, licenza GPL): la finestra che vedi è un PC virtuale
...
4. **Packages** — carica `WindiroOS-x86_64.msix` (NON serve firma: la mette Microsoft)
5. **Store listing** — descrizione, screenshots del desktop WindiroOS, logo
...
- `windiroos_launcher.c` — launcher Win32 (GUI dark, Avvia/Stop, a schermo intero)
- `windiroos.rc` — icona + info versione dell'exe
- `AppxManifest.xml` — manifest MSIX (placeholder da sostituire)
- `gen_assets.py` — genera icone PNG/ICO (nessuna dipendenza)
- `windiroos.nsi` — installer NSIS (GitHub)
